...
import (
	...
	"github.com/qdrddr/chunk-your-skills/sdk/go/v2/moduleversion"
)

func TestResolveVersionUsesModuleVersion(t *testing.T) {
	t.Setenv("CHUNK_YOUR_SKILLS_RELEASE_VERSION", "")
...
func TestIsSDKModuleRoot(t *testing.T) {
	...
	if err := os.WriteFile(goMod, []byte("module github.com/qdrddr/chunk-your-skills/sdk/go/v2\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestLookupChecksum(t *testing.T) {
	sumData := []byte("abc123  chunk-your-skills-ffi-aarch64-apple-darwin.tar.gz\n")
	got, err := lookupChecksum(sumData, "chunk-your-skills-ffi-aarch64-apple-darwin.tar.gz")
	if err != nil {
		t.Fatal(err)
	}
	if got != "abc123" {
...
func TestCopyArtifactsIncludesSharedByDefault(t *testing.T) {
	...
	for name, content := range map[string]string{
		"libchunk_your_skills.a":     "static",
		"libchunk_your_skills.dylib": "shared",
	...
	if _, err := os.Stat(filepath.Join(dest, "libchunk_your_skills.dylib")); err != nil {
		t.Fatal("expected shared lib when staticOnly is false")
	}
}

func TestEnsureNativeDownloadsWhenCacheEmpty(t *testing.T) {
	if os.Getenv("CHUNK_YOUR_SKILLS_NATIVE_ENSURE_INTEGRATION") != "1" {
		t.Skip("set CHUNK_YOUR_SKILLS_NATIVE_ENSURE_INTEGRATION=1 to run release download test")
	}
	cacheRoot := t.TempDir()
...
func TestHasNativeLibsWindowsImportLib(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "chunk_your_skills.dll.lib"), []byte("import"), 0o644); err != nil {
		t.Fatal(err)
	}
	if !hasNativeLibs(dir, triplet) {
		t.Fatal("expected chunk_your_skills.dll.lib to satisfy hasNativeLibs on Windows")
	}
}
...
func TestResolveTripletUsesEnv(t *testing.T) {
	t.Setenv("CHUNK_YOUR_SKILLS_RUST_TARGET", "aarch64-unknown-linux-gnu")
...
func TestCopyArtifactsStaticOnly(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dest, "chunk_your_skills.h")); err != nil {
		t.Fatal("missing header")
	}
	if _, err := os.Stat(filepath.Join(dest, "libchunk_your_skills.dylib")); !os.IsNotExist(err) {
