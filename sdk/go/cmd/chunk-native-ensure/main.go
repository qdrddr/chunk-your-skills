// Command chunk-native-ensure downloads or copies chunk-your-skills C FFI artifacts for the host platform.
package main
...
func main() {
	var (
		version    = flag.String("version", "", "Release semver (default: CHUNK_YOUR_SKILLS_RELEASE_VERSION, then module version)")
		...
		cacheDir   = flag.String("cache-dir", "", "Override cache root (default: XDG_CACHE_HOME/chunk-your-skills)")
	...
	fmt.Printf("chunk-your-skills native artifacts ready: %s\n", dest)
...
func resolveVersion(flagVersion string) string {
	...
	if env := os.Getenv("CHUNK_YOUR_SKILLS_RELEASE_VERSION"); env != "" {
...
func cacheDestDir(cacheRoot, version, triplet string) (string, error) {
	...
	if env := os.Getenv("CHUNK_YOUR_SKILLS_NATIVE_DIR"); env != "" {
	...
	return filepath.Join(base, "chunk-your-skills", version, triplet), nil
}
...
func repoRootFromCwd() string {
	for _, root := range []string{
		os.Getenv("CHUNK_YOUR_SKILLS_REPO_ROOT"),
