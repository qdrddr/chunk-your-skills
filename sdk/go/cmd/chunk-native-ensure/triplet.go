...
const defaultRepo = "qdrddr/chunk-your-skills"
...
func archiveName(triplet string) string {
	return "chunk-your-skills-ffi-" + triplet + ".tar.gz"
}

func staticLibName(triplet string) string {
	if isWindowsMSVC(triplet) {
		return "chunk_your_skills.lib"
	}
...
func sharedLibName(triplet string) string {
	...
	case isWindowsMSVC(triplet):
		return "chunk_your_skills.dll"
	case isDarwin(triplet):
		return "libchunk_your_skills.dylib"
	default:
		return "libchunk_your_skills.so"
	}
}

func importLibName(triplet string) string {
	if isWindowsMSVC(triplet) {
		return "chunk_your_skills.dll.lib"
	}
	return ""
}
...
func resolveTriplet() (string, error) {
	if env := strings.TrimSpace(os.Getenv("CHUNK_YOUR_SKILLS_RUST_TARGET")); env != "" {
