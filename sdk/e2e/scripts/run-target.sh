...
# Usage: CHUNK_YOUR_SKILLS_RELEASE_VERSION=x.y.z ./run-target.sh <rust|python|typescript|go|c>
set -euo pipefail
...
if [[ -z "$TARGET" ]]; then
	echo "usage: CHUNK_YOUR_SKILLS_RELEASE_VERSION=x.y.z $0 <rust|python|typescript|go|c>" >&2
	exit 1
fi
...
export_native_lib_path() {
	local staging="${CHUNK_YOUR_SKILLS_E2E_STAGING:?run prepare-release-checkout.sh first}"
	local triplet="${CHUNK_YOUR_SKILLS_RUST_TARGET:-$("${ROOT}/scripts/host-rust-target.sh")}"
...
prepare_go_c() {
	_cys_e2e_staging="$("${ROOT}/scripts/prepare-release-checkout.sh")"
	export CHUNK_YOUR_SKILLS_E2E_STAGING="$_cys_e2e_staging"
	unset _cys_e2e_staging
...
rust)
	echo "=== Rust (crates.io) ==="
	maybe_wait "crates.io/chunk-your-skills" crate
...
go)
	...
	maybe_wait "GitHub tag v${CHUNK_YOUR_SKILLS_RELEASE_VERSION}" tag
	maybe_wait "GitHub Release C FFI v${CHUNK_YOUR_SKILLS_RELEASE_VERSION}" release-assets
	prepare_go_c
	export_native_lib_path
...
c)
	...
	maybe_wait "GitHub Release C FFI v${CHUNK_YOUR_SKILLS_RELEASE_VERSION}" release-assets
	prepare_go_c
	C_SDK="${CHUNK_YOUR_SKILLS_E2E_STAGING}/sdk/c"
	export CARGO_TARGET_DIR="${CHUNK_YOUR_SKILLS_E2E_STAGING}/target"
	cmake -S "${C_SDK}" -B "${C_SDK}/build" -DCMAKE_BUILD_TYPE=Release \
		-DCYS_RUST_TARGET="${CHUNK_YOUR_SKILLS_RUST_TARGET:-$("${ROOT}/scripts/host-rust-target.sh")}"
