...
crate_has_version() {
	local ver="$1"
	local url="https://crates.io/api/v1/crates/chunk-your-skills/${ver}"
...
npm_has_version() {
	local ver="$1"
	npm view "chunk-your-skills@${ver}" version 2>/dev/null | grep -qxF "$ver"
}

tag_has_version() {
	local ver="$1"
	local tag="v${ver}"
	local repo="${CYS_E2E_GIT_REPO:-https://github.com/qdrddr/chunk-your-skills.git}"
...
release_has_ffi_assets() {
	local ver="$1"
	local tag="v${ver}"
	local slug="${CYS_E2E_GITHUB_REPO:-qdrddr/chunk-your-skills}"
	local url="https://api.github.com/repos/${slug}/releases/tags/${tag}"
	local json
	json="$(curl -fsSL "$url")"
	local expected=(
		chunk-your-skills-ffi-x86_64-unknown-linux-gnu.tar.gz
		chunk-your-skills-ffi-aarch64-unknown-linux-gnu.tar.gz
		chunk-your-skills-ffi-x86_64-apple-darwin.tar.gz
		chunk-your-skills-ffi-aarch64-apple-darwin.tar.gz
		chunk-your-skills-ffi-x86_64-pc-windows-msvc.tar.gz
		chunk-your-skills-ffi-aarch64-pc-windows-msvc.tar.gz
		SHA256SUMS
		chunk_your_skills.h
	)
	local name
...
case "$TARGET" in
crate)
	wait_loop "crates.io/chunk-your-skills" crate_has_version "$VERSION"
	;;
pypi-sdk)
...
npm)
	...
	;;
tag)
	wait_loop "GitHub/chunk-your-skills" tag_has_version "$VERSION"
	;;
release-assets)
