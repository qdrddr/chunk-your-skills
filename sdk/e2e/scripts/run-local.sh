...
Environment:
  CHUNK_YOUR_SKILLS_RELEASE_VERSION   Same as VERSION
  TAG                   Release tag to parse (e.g. v0.1.10)
  SKIP_REGISTRY_WAIT=1  Same as --skip-wait
  CHUNK_YOUR_SKILLS_E2E_USE_WORKSPACE=1  Same as --workspace
...
resolve_version() {
	if [[ -n "${CHUNK_YOUR_SKILLS_RELEASE_VERSION:-}" ]]; then
		return 0
	fi
	if [[ -n "${TAG:-}" ]]; then
	...
	export CHUNK_YOUR_SKILLS_RELEASE_VERSION="$ws_version"
	echo "Using workspace version CHUNK_YOUR_SKILLS_RELEASE_VERSION=${CHUNK_YOUR_SKILLS_RELEASE_VERSION}"
}

SKIP_WAIT="${SKIP_REGISTRY_WAIT:-0}"
USE_WORKSPACE="${CHUNK_YOUR_SKILLS_E2E_USE_WORKSPACE:-0}"
TARGETS=()
...
export CHUNK_YOUR_SKILLS_E2E_USE_WORKSPACE="$USE_WORKSPACE"
...
echo "Registry E2E: CHUNK_YOUR_SKILLS_RELEASE_VERSION=${CHUNK_YOUR_SKILLS_RELEASE_VERSION}"
