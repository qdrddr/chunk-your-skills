#!/usr/bin/env bash
# Build libchunk_your_skills in CHUNK_YOUR_SKILLS_E2E_STAGING for Go/C E2E harnesses.
set -euo pipefail
...
PROFILE="${CHUNK_YOUR_SKILLS_C_LIB_PROFILE:-release}"
...
if [[ ! -f "${STAGING}/Cargo.toml" ]]; then
	echo "::error::missing Cargo.toml in CHUNK_YOUR_SKILLS_E2E_STAGING=${STAGING}" >&2
	exit 1
fi
