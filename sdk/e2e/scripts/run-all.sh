...
if [[ -z "${CHUNK_YOUR_SKILLS_RELEASE_VERSION:-}" && -n "${TAG:-}" ]]; then
	# shellcheck source=parse-version.sh
	eval "$("${ROOT}/scripts/parse-version.sh")"
fi

export CHUNK_YOUR_SKILLS_RELEASE_VERSION="${CHUNK_YOUR_SKILLS_RELEASE_VERSION:?set CHUNK_YOUR_SKILLS_RELEASE_VERSION or TAG}"
