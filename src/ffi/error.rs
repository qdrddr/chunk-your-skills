...
pub const ERR_ALLOC: c_int = -4;
/// I/O or filesystem error.
pub const ERR_IO: c_int = -5;
/// Invalid opaque handle.
pub const ERR_INVALID_HANDLE: c_int = -6;
    ...
    LAST_ERROR.with(|e| {
        if let Ok(mut err) = e.try_borrow_mut() {
            *err = CString::new(msg).ok();
            ...
            eprintln!("FATAL: RefCell panic in chunk-your-skills error handling");
            std::process::abort();
        }
    });
}
    ...
    LAST_ERROR.with(|e| {
        if let Ok(mut err) = e.try_borrow_mut() {
            *err = None;
            ...
            eprintln!("FATAL: RefCell panic in chunk-your-skills error clearing");
            std::process::abort();
        }
    });
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chunk_your_skills_get_last_error() -> *const c_char {
    LAST_ERROR.with(|e| {
        (*e.borrow())
            .as_ref()
...
/// No pointer arguments; safe to call from C when linked against this library.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chunk_your_skills_clear_error() {
    clear_error();
}
