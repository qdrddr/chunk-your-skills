...
use crate::ffi::error::{ERR_ALLOC, ERR_NULL_PTR, OK, clear_error, set_error};
use std::ffi::CString;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chunk_your_skills_free_string(s: *mut c_char) {
    if !s.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn chunk_your_skills_get_version(out: *mut *mut c_char) -> c_int {
    if out.is_null() {
