...
read_cmake_project_version() {
	grep -E '^project\(chunk-your-skills-c VERSION ' "${REPO_ROOT}/sdk/c/CMakeLists.txt" |
		head -1 |
		sed -E 's/^project\(chunk-your-skills-c VERSION ([^ ]+) .*/\1/'
}
...
dependency ranges in sdk/python/pyproject.toml, package.json files, and Cargo.toml.
...
report_c_inventory() {
	...
	info "c sdk: export manifest inventory"
	{
		...
		printf 'public_header=sdk/c/include/chunk_your_skills.h\n'
		printf 'supported_targets=\n'
		awk '
			/^_cys_supported_targets/ { in_list=1; next }
...
verify_c_sdk() {
	...
	local header_src="${REPO_ROOT}/chunk_your_skills.h"
	local header_dest="${c_dir}/include/chunk_your_skills.h"
	...
	[[ -f "${header_dest}" ]] ||
		die "c sdk: missing include/chunk_your_skills.h"
	[[ -f "${header_src}" ]] ||
		die "c sdk: missing root chunk_your_skills.h (run: cargo build -p chunk-your-skills --features ffi)"
...
lint_cmake_project_version() {
	...
	if [[ -z "${cmake_version}" ]]; then
		append_manifest_lint "${file}: missing project(chunk-your-skills-c VERSION ...)"
...
verify_manifests() {
	...
	lint_pyproject_ranges "pyproject.toml (sdk/python)" \
		"${REPO_ROOT}/sdk/python/pyproject.toml" || had_failure=1

	lint_package_json_ranges "package.json (root)" "${REPO_ROOT}/package.json" || had_failure=1
	lint_package_json_ranges "package.json (sdk/typescript)" \
		"${REPO_ROOT}/sdk/typescript/package.json" || had_failure=1

	lint_cargo_toml_ranges "Cargo.toml (workspace)" "${REPO_ROOT}/Cargo.toml" || had_failure=1
